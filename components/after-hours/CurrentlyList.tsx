...
export function CurrentlyList() {
  ...
  return (
    <div className="mb-16 flex flex-col gap-3">
      <span className="text-[12px] font-semibold tracking-[0.1em] text-ink-num">CURRENTLY</span>
      <div className="grid grid-cols-2 gap-3 tab:grid-cols-4">
        {CURRENTLY.map((item) => (
          <div key={item.label} className="flex flex-col gap-1 rounded-lg border border-line-strong bg-surface px-4 py-3">
            <span className="text-[12px] font-medium tracking-[0.05em] text-ink-faint">{item.label}</span>
