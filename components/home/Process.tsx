        ...
        <div className="grid grid-cols-1 items-start gap-8 tab:grid-cols-[minmax(0,38%)_minmax(0,58%)] tab:justify-between tab:gap-10 desk:grid-cols-[minmax(0,36%)_minmax(0,60%)] desk:gap-12">
          ...
          <div className="flex flex-col">
            {PROCESS.map((step) => (
              ...
              >
                <span className="flex items-start gap-[5px] text-p2 font-medium tracking-[-0.01em]">
                  {step.title}
                  <span className="pt-0.5 text-meta text-ink-num transition-colors duration-[260ms] ease-linear group-hover:text-accent group-focus-visible:text-accent">
                    {step.num}
                  </span>
                </span>
                <p className="m-0 max-w-[68ch] text-p2 text-ink-muted text-pretty transition-colors duration-[260ms] ease-linear group-hover:text-ink-secondary group-focus-visible:text-ink-secondary">
                  {step.body}
                </p>
              </div>
            ))}
